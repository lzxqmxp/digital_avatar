      ...
      <div class="form-col">
        ...
        <div class="form-row">
          <input
            ref="importInput"
            type="file"
            accept=".pth,.onnx,.engine"
            style="display: none"
            @change="onImportFile"
          />
          <button
            id="btn_model_import"
            :class="btnClass('btn_model_import')"
            :disabled="!newModelName.trim() || btnStates['btn_model_import'] === 'loading'"
            @click="onImportClick"
          >
        ...
        <div class="msg-row">
          <span
            v-if="btnMessages['btn_model_import']"
            :class="btnStates['btn_model_import'] === 'error' ? 'msg--error' : 'msg--success'"
            >{{ btnMessages['btn_model_import'] }}</span
          >
        </div>
      </div>
    </section>
    ...
    <section class="section">
      ...
      <div v-for="m in models" :key="m.id" class="model-item">
        <div class="model-info">
          ...
          <span class="status-badge" :style="{ color: statusColor[m.status] || '#94a3b8' }">{{
            m.status
          }}</span>
        </div>
        ...
        <div class="model-actions">
          <button
            :id="`btn_model_verify_${m.id}`"
            :class="btnClass(`btn_model_verify_${m.id}`)"
            :disabled="
              m.status === 'validated' ||
              m.status === 'active' ||
              btnStates[`btn_model_verify_${m.id}`] === 'loading'
            "
            @click="onVerify(m.id)"
          >
            校验
          ...
          <button
            :id="`btn_model_enable_${m.id}`"
            :class="btnClass(`btn_model_enable_${m.id}`)"
            :disabled="
              m.status !== 'validated' || btnStates[`btn_model_enable_${m.id}`] === 'loading'
            "
            @click="onEnable(m.id)"
          >
            启用
          ...
          <button
            :id="`btn_model_rollback_${m.id}`"
            :class="btnClass(`btn_model_rollback_${m.id}`)"
            :disabled="
              m.status !== 'active' || btnStates[`btn_model_rollback_${m.id}`] === 'loading'
            "
          ...
          <button
            :id="`btn_model_delete_${m.id}`"
            :class="[...btnClass(`btn_model_delete_${m.id}`), 'btn--danger']"
            :disabled="m.status === 'active' || btnStates[`btn_model_delete_${m.id}`] === 'loading'"
            @click="onDelete(m.id)"
          >
            删除
          </button>
        ...
        <div class="msg-row">
          <span v-for="suffix in ['verify', 'enable', 'rollback', 'delete']" :key="suffix">
            <span
              v-if="btnMessages[`btn_model_${suffix}_${m.id}`]"
              :class="
                btnStates[`btn_model_${suffix}_${m.id}`] === 'error' ? 'msg--error' : 'msg--success'
              "
              >{{ btnMessages[`btn_model_${suffix}_${m.id}`] }}</span
            >
          </span>
        </div>
      </div>
