...
async function onGenerate() {
  ...
  if (res.ok && res.data) {
    ...
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_generate', action: '生成话术', result: 'success' })
  } else {
    setBtn('btn_writer_generate', 'error', res.message || '生成失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_generate', action: '生成话术', result: 'failure', error_code: res.errorCode })
  }
}
...
async function onRewrite() {
  ...
  const res = await callApi<WriterRewriteResponse>(ApiPaths.WRITER_REWRITE, 'POST', { text: selectedCandidate.value })
  if (res.ok && res.data) {
    rewrittenText.value = res.data.rewritten
    selectedCandidate.value = res.data.rewritten
    setBtn('btn_writer_rewrite', 'success', '改写完成')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_rewrite', action: '改写话术', result: 'success' })
  } else {
    setBtn('btn_writer_rewrite', 'error', res.message || '改写失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_rewrite', action: '改写话术', result: 'failure', error_code: res.errorCode })
  }
}
...
async function onSensitiveCheck() {
  ...
  sensitiveHits.value = []
  const res = await callApi<WriterSensitiveCheckResponse>(ApiPaths.WRITER_SENSITIVE_CHECK, 'POST', { text: selectedCandidate.value })
  if (res.ok && res.data) {
    ...
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_sensitive_check', action: '敏感词检测', result: res.data.safe ? 'success' : 'failure' })
  } else {
    setBtn('btn_writer_sensitive_check', 'error', res.message || '检测失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_sensitive_check', action: '敏感词检测', result: 'failure', error_code: res.errorCode })
  }
}
...
async function onSaveDraft() {
  ...
  if (res.ok && res.data) {
    ...
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_save_draft', action: '保存草稿', result: 'success' })
  } else {
    setBtn('btn_writer_save_draft', 'error', res.message || '保存失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_save_draft', action: '保存草稿', result: 'failure', error_code: res.errorCode })
  }
}
...
async function onPublish() {
  ...
  const res = await callApi<WriterPublishResponse>(ApiPaths.WRITER_PUBLISH, 'POST', { draft_id: savedDraftId.value })
  if (res.ok && res.data) {
    ...
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_publish', action: '发布到话术库', result: 'success' })
  } else {
    setBtn('btn_writer_publish', 'error', res.message || '发布失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_writer_publish', action: '发布到话术库', result: 'failure', error_code: res.errorCode })
  }
}
</script>
  ...
  <div class="page">
    ...
    <section class="section">
      ...
      <div class="form-col">
        ...
        <div class="form-row">
          <textarea v-model="inputText" class="input textarea" placeholder="输入参考内容或关键词 (max 500)" maxlength="500" rows="3" />
        </div>
        <div class="form-row">
          <button id="btn_writer_generate" :class="btnClass('btn_writer_generate')" :disabled="btnStates['btn_writer_generate'] === 'loading'" @click="onGenerate">
            {{ btnStates['btn_writer_generate'] === 'loading' ? '生成中...' : '生成' }}
          </button>
        </div>
        <div class="msg-row">
          <span v-if="btnMessages['btn_writer_generate']" :class="btnStates['btn_writer_generate'] === 'error' ? 'msg--error' : 'msg--success'">{{ btnMessages['btn_writer_generate'] }}</span>
        </div>
      </div>
    </section>
    ...
    <section class="section" v-if="candidates.length > 0">
      ...
      <div class="candidate-list">
        <div v-for="(c, i) in candidates" :key="i" class="candidate-item" :class="{ 'candidate-item--selected': selectedCandidate === c }" @click="selectedCandidate = c">
          {{ c }}
        </div>
      </div>
    </section>
    ...
    <section class="section">
      ...
      <div class="form-col">
        <textarea v-model="selectedCandidate" class="input textarea" placeholder="在此输入或选择候选文案" rows="3" />
        <div v-if="sensitiveHits.length > 0" class="sensitive-box">
          命中敏感词：<span v-for="w in sensitiveHits" :key="w" class="sensitive-word">{{ w }}</span>
        </div>
        <div class="form-row">
          <button id="btn_writer_rewrite" :class="btnClass('btn_writer_rewrite')" :disabled="!selectedCandidate.trim() || btnStates['btn_writer_rewrite'] === 'loading'" @click="onRewrite">改写</button>
          <button id="btn_writer_sensitive_check" :class="btnClass('btn_writer_sensitive_check')" :disabled="!selectedCandidate.trim() || btnStates['btn_writer_sensitive_check'] === 'loading'" @click="onSensitiveCheck">敏感词检测</button>
          <button id="btn_writer_save_draft" :class="btnClass('btn_writer_save_draft')" :disabled="!selectedCandidate.trim() || btnStates['btn_writer_save_draft'] === 'loading'" @click="onSaveDraft">保存草稿</button>
          <button id="btn_writer_publish" :class="btnClass('btn_writer_publish')" :disabled="!savedDraftId || btnStates['btn_writer_publish'] === 'loading'" @click="onPublish">发布到话术库</button>
        </div>
        <div class="msg-row">
          <span v-for="id in ['btn_writer_rewrite','btn_writer_sensitive_check','btn_writer_save_draft','btn_writer_publish']" :key="id">
            <span v-if="btnMessages[id]" :class="btnStates[id] === 'error' ? 'msg--error' : 'msg--success'">{{ btnMessages[id] }}</span>
          </span>
        </div>
        <div v-if="savedDraftId" class="status-chip chip--blue">草稿已保存: {{ savedDraftId.slice(0, 8) }}</div>
        <div v-if="publishedScriptId" class="status-chip chip--green">已发布: {{ publishedScriptId.slice(0, 8) }}</div>
      </div>
    </section>
  </div>
</template>
...
.input { background: #0f172a; border: 1px solid #334155; color: #e2e8f0; padding: 6px 10px; border-radius: 6px; font-size: 13px; }
...
.field-label { font-size: 12px; color: #94a3b8; white-space: nowrap; }
.candidate-list { display: flex; flex-direction: column; gap: 6px; }
.candidate-item { padding: 8px 12px; background: #0f172a; border-radius: 6px; font-size: 13px; color: #e2e8f0; cursor: pointer; border: 1px solid #334155; transition: border-color 0.15s; }
.candidate-item:hover { border-color: #475569; }
.candidate-item--selected { border-color: #38bdf8; }
.sensitive-box { background: #450a0a; border: 1px solid #b91c1c; border-radius: 6px; padding: 8px 12px; font-size: 12px; color: #fca5a5; }
.sensitive-word { background: #b91c1c; color: #fecaca; padding: 1px 6px; border-radius: 4px; margin-left: 4px; }
.status-chip { display: inline-block; font-size: 11px; padding: 2px 8px; border-radius: 99px; margin-top: 4px; }
