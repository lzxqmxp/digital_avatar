...
async function onEnable() {
  for (const id of selectedIds.value) {
    const res = await callApi<ScriptStatusResponse>(ApiPaths.SCRIPTS_STATUS, 'POST', {
      id,
      status: 'enabled' as ScriptStatus
    })
    if (res.ok && res.data) {
      const idx = scripts.value.findIndex((s) => s.id === id)
      if (idx !== -1) scripts.value[idx] = res.data
...
async function onDisable() {
  for (const id of selectedIds.value) {
    const res = await callApi<ScriptStatusResponse>(ApiPaths.SCRIPTS_STATUS, 'POST', {
      id,
      status: 'disabled' as ScriptStatus
    })
    if (res.ok && res.data) {
      const idx = scripts.value.findIndex((s) => s.id === id)
      if (idx !== -1) scripts.value[idx] = res.data
...
async function onImportFile(e: Event) {
  ...
  if (!['csv', 'xlsx', 'json'].includes(ext || '')) {
    ...
    writeAudit({
      session_id: sessionStore.sessionId || '',
      button_id: 'btn_script_import',
      action: '导入话术',
      result: 'failure',
      error_code: ErrorCode.FORMAT_NOT_SUPPORTED
    })
    return
  }
...
const statusColor: Record<ScriptStatus, string> = {
  draft: '#94a3b8',
  enabled: '#4ade80',
  disabled: '#f87171'
}
...
<template>
  <div class="page">
      ...
      <div class="form-col">
        ...
        <div class="form-row">
          <input
            ...
            placeholder="内容 (max 120)"
            maxlength="120"
          />
          <button
            id="btn_script_new"
            :class="btnClass('btn_script_new')"
            :disabled="btnStates['btn_script_new'] === 'loading'"
            @click="onNew"
          >
            新建
          </button>
        ...
        <div class="msg-row">
          <span
            v-if="btnMessages['btn_script_new']"
            :class="btnStates['btn_script_new'] === 'error' ? 'msg--error' : 'msg--success'"
            >{{ btnMessages['btn_script_new'] }}</span
          >
        </div>
      </div>
    </section>
    ...
    <section class="section">
      ...
      <div class="form-row">
        <button
          id="btn_script_enable"
          :class="btnClass('btn_script_enable')"
          :disabled="selectedIds.size === 0 || btnStates['btn_script_enable'] === 'loading'"
          @click="onEnable"
        >
          启用
        </button>
        <button
          id="btn_script_disable"
          :class="btnClass('btn_script_disable')"
          :disabled="selectedIds.size === 0 || btnStates['btn_script_disable'] === 'loading'"
          @click="onDisable"
        >
          禁用
        </button>
        <button
          id="btn_script_delete"
          :class="btnClass('btn_script_delete')"
          :disabled="selectedIds.size === 0 || btnStates['btn_script_delete'] === 'loading'"
          @click="onDelete"
        >
          删除
        </button>
        ...
        />
        <button
          id="btn_script_import"
          :class="btnClass('btn_script_import')"
          :disabled="btnStates['btn_script_import'] === 'loading'"
          @click="onImportClick"
        >
          导入
        </button>
        <button
          id="btn_script_export"
          :class="btnClass('btn_script_export')"
          :disabled="scripts.length === 0 || btnStates['btn_script_export'] === 'loading'"
          @click="onExport"
        >
          导出
        </button>
    ...
    <section class="section">
      ...
      <div
        ...
        :key="item.id"
        class="script-item"
        :class="{ 'script-item--selected': selectedIds.has(item.id) }"
      >
        ...
        <div class="script-body">
          ...
          <div v-else class="script-edit">
            ...
            <button
              id="btn_script_save"
              :class="btnClass('btn_script_save')"
              :disabled="btnStates['btn_script_save'] === 'loading'"
      ...
      <div class="msg-row" v-if="btnMessages['btn_script_save']">
        <span :class="btnStates['btn_script_save'] === 'error' ? 'msg--error' : 'msg--success'">{{
          btnMessages['btn_script_save']
        }}</span>
      </div>
    </section>
  </div>
