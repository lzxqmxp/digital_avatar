...
async function onNew() {
  ...
  const res = await callApi<PolicyCreateResponse>(ApiPaths.POLICIES_CREATE, 'POST', { name: newPolicyName.value.trim() })
  if (res.ok && res.data) {
    policies.value.push(res.data)
    selectPolicy(res.data)
    newPolicyName.value = ''
    setBtn('btn_policy_new', 'success', '新建成功')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_new', action: '新建策略', result: 'success' })
  } else {
    setBtn('btn_policy_new', 'error', res.message || '新建失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_new', action: '新建策略', result: 'failure', error_code: res.errorCode })
  }
}
...
async function onSave() {
  ...
  if (res.ok && res.data) {
    ...
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_save', action: '保存策略', result: 'success' })
  } else {
    setBtn('btn_policy_save', 'error', res.message || '保存失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_save', action: '保存策略', result: 'failure', error_code: res.errorCode })
  }
}
...
async function onTest() {
  ...
  if (res.ok && res.data) {
    ...
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_test', action: '策略测试', result: 'success' })
  } else {
    setBtn('btn_policy_test', 'error', res.message || '测试失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_test', action: '策略测试', result: 'failure', error_code: res.errorCode })
  }
}
...
async function onPublish() {
  ...
  const res = await callApi<PolicyPublishResponse>(ApiPaths.POLICIES_PUBLISH, 'POST', { id: selectedPolicy.value.id })
  if (res.ok && res.data) {
    await loadPolicies()
    setBtn('btn_policy_publish', 'success', '策略已生效')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_publish', action: '发布策略', result: 'success' })
  } else {
    setBtn('btn_policy_publish', 'error', res.message || '发布失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_publish', action: '发布策略', result: 'failure', error_code: res.errorCode })
  }
}
...
async function onRollback() {
  ...
  if (res.ok && res.data) {
    ...
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_rollback', action: '回滚策略', result: 'success' })
  } else {
    setBtn('btn_policy_rollback', 'error', res.message || '回滚失败')
    writeAudit({ session_id: sessionStore.sessionId || '', button_id: 'btn_policy_rollback', action: '回滚策略', result: 'failure', error_code: res.errorCode })
  }
}

const statusColor: Record<string, string> = { active: '#4ade80', inactive: '#94a3b8', draft: '#fbbf24' }
...
<template>
  <div class="page">
    ...
    <section class="section">
      ...
      <div class="form-row">
        <input v-model="newPolicyName" class="input" placeholder="新策略名称 (max 40)" maxlength="40" />
        <button id="btn_policy_new" :class="btnClass('btn_policy_new')" :disabled="btnStates['btn_policy_new'] === 'loading'" @click="onNew">新建策略</button>
      </div>
      <div class="msg-row">
        <span v-if="btnMessages['btn_policy_new']" :class="btnStates['btn_policy_new'] === 'error' ? 'msg--error' : 'msg--success'">{{ btnMessages['btn_policy_new'] }}</span>
      </div>
      <div class="policy-list">
        <div v-for="p in policies" :key="p.id" class="policy-item" :class="{ 'policy-item--active': selectedPolicy?.id === p.id }" @click="selectPolicy(p)">
          <span class="policy-name">{{ p.name }}</span>
          <span class="policy-version">v{{ p.version }}</span>
          <span class="status-dot" :style="{ backgroundColor: statusColor[p.status] || '#94a3b8' }" />
    ...
    <section class="section" v-if="selectedPolicy">
      ...
      <div class="form-col">
        <div class="form-row">
          ...
          <input v-model.number="editTemp" class="input input--sm" type="number" step="0.1" min="0.2" max="1.2" />
          <label class="field-label">最大回复长度 (20-120)</label>
          <input v-model.number="editMaxLen" class="input input--sm" type="number" step="1" min="20" max="120" />
        </div>
        <div class="form-row">
          <label class="field-label">风控模式</label>
          <select v-model="editRiskMode" class="input">
            <option v-for="opt in riskModeOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
          </select>
          <button id="btn_policy_save" :class="btnClass('btn_policy_save')" :disabled="btnStates['btn_policy_save'] === 'loading'" @click="onSave">保存策略</button>
          <button id="btn_policy_publish" :class="btnClass('btn_policy_publish')" :disabled="btnStates['btn_policy_publish'] === 'loading'" @click="onPublish">发布策略</button>
        </div>
        <div class="msg-row">
        ...
        <div class="form-row">
          ...
          <input v-model.number="rollbackVersion" class="input input--sm" type="number" step="1" min="1" :max="selectedPolicy.version - 1" />
          <button id="btn_policy_rollback" :class="btnClass('btn_policy_rollback')" :disabled="selectedPolicy.version <= 1 || btnStates['btn_policy_rollback'] === 'loading'" @click="onRollback">回滚策略</button>
        </div>
        <div class="msg-row">
          <span v-if="btnMessages['btn_policy_rollback']" :class="btnStates['btn_policy_rollback'] === 'error' ? 'msg--error' : 'msg--success'">{{ btnMessages['btn_policy_rollback'] }}</span>
        </div>
      </div>
    </section>
    ...
    <section class="section" v-if="selectedPolicy">
      ...
      <div class="form-col">
        <div class="form-row">
          ...
          <button id="btn_policy_test" :class="btnClass('btn_policy_test')" :disabled="btnStates['btn_policy_test'] === 'loading'" @click="onTest">策略测试</button>
        </div>
        <div v-if="testReply" class="reply-box">{{ testReply }}</div>
        <div class="msg-row">
          <span v-if="btnMessages['btn_policy_test']" :class="btnStates['btn_policy_test'] === 'error' ? 'msg--error' : 'msg--success'">{{ btnMessages['btn_policy_test'] }}</span>
        </div>
      </div>
    </section>
  </div>
...
.empty { color: #64748b; font-size: 13px; text-align: center; padding: 16px 0; }
.policy-list { display: flex; flex-direction: column; gap: 6px; margin-top: 12px; }
.policy-item { display: flex; align-items: center; gap: 10px; padding: 8px 12px; background: #0f172a; border-radius: 6px; cursor: pointer; border: 1px solid #334155; transition: border-color 0.15s; }
.policy-item--active { border-color: #38bdf8; }
.policy-item:hover { border-color: #475569; }
.policy-name { flex: 1; font-size: 13px; font-weight: 600; color: #e2e8f0; }
.policy-version { font-size: 11px; color: #94a3b8; }
.status-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.policy-status { font-size: 11px; color: #94a3b8; }
.reply-box { background: #0f172a; border: 1px solid #334155; border-radius: 6px; padding: 10px 12px; font-size: 13px; color: #94a3b8; min-height: 40px; }
</style>
